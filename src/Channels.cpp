...
std::string Channels::GetChannelIcon(int channelID)
{
  ...
  if (kodi::vfs::FileExists(kodi::vfs::TranslateSpecialProtocol(iconFilename), false))
  {
    return iconFilename;
  }
...
void Channels::DeleteChannelIcons()
{
  ...
  if (kodi::vfs::GetDirectory("special://userdata/addon_data/pvr.nextpvr/","nextpvr-ch*.png", icons))
  {
    ...
    for (auto const& it : icons)
    {
      ...
      kodi::Log(ADDON_LOG_DEBUG, "DeleteFile %s rc:%d", kodi::vfs::TranslateSpecialProtocol(deleteme), remove(kodi::vfs::TranslateSpecialProtocol(deleteme).c_str()));
    }
  }
}
