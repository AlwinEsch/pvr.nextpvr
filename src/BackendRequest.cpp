...
namespace NextPVR
{
  int Request::DoRequest(const char* resource, std::string& response)
  {
    ...
    if (stream.OpenFile(URL, READ_NO_CACHE))
    {
      char buffer[1025] = {0};
      int count;
  ...
  int Request::FileCopy(const char* resource, std::string fileName)
  {
    ...
    ssize_t datalen;
    if (inputStream.OpenFile(URL, READ_NO_CACHE))
    {
      kodi::vfs::CFile outputFile;
      if (outputFile.OpenFileForWrite(fileName, READ_NO_CACHE))
      {
        char buffer[1024];
  ...
  bool Request::PingBackend()
  {
    ...
    if (backend.OpenFile(URL, READ_NO_CACHE))
    {
      backend.Close();
      return true;
    }
